...
pipeline {
  ...
  stages {
    ...
    stage('Staging Deploy') {
      steps {
        sh 'sudo cp target/*/*.jar /opt/deploy/batch_ml/'
        sh 'sudo cp -Rf conf/* /opt/deploy/batch_ml/'
    ...
    stage('Production Deploy') {
      steps {
        ...
        sh "sudo ansible-playbook -i \'${DEPLOY_TARGET},\' --private-key=/home/xxpasquxx/.ssh/ansible_rsa_key /opt/DevOpsProduction-Orchestrator/ansible/deploy/batch_ml_deploy.yml  -e \'ansible_ssh_user=xxpasquxx\' -e \'host_key_checking=False\'"
      }
    }
  }
  post {
